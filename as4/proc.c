...
found:
  ...
  *(uint *)sp = (uint)trapret;
  ...
  // Set date and new props to default
...
int sys_cps(void)
{
  ...
  for (i = 0; i < NPROC; i++)
  {
    if (ptable.proc[i].state != UNUSED)
    {
      ...
      int year = ptable.proc[i].begin_date.year;
      int month = ptable.proc[i].begin_date.month;
      int day = ptable.proc[i].begin_date.day;
      int hour = ptable.proc[i].begin_date.hour;
